/*
GROUP SUBMISSION BY:
1. PRATHAMESH SAI SANKAR (STUDENT ID: 19314123)
2. TOMASZ BOGUN (STUDENT ID: 19335070)
*/
...
void student_conv(float ***image, int16_t ****kernels, float ***output,
                  ...
                  int kernel_order)
{
  //Convert the kernels vector (short int) to a float vector.
  float ****kernelsAsFloats = new_empty_4d_matrix_float(nkernels, kernel_order, kernel_order, nchannels);
  ...
  for (m = 0; m < nkernels; m++){
    for (w = 0; w < width; w++){
      for (h = 0; h < height; h++){
        ...
        for (x = 0; x < kernel_order; x++){
          for (y = 0; y < kernel_order; y++){
            //Using OpenMP to vectorize inner nested-loop instead of SSE.
            #pragma omp simd
            //Increment of 16 used as it is optimal for speedup (see report)
            for (c = 0; c < nchannels; c += 16){
              sum += image[w+x][h+y][c] * kernelsAsFloats[m][x][y][c];
              sum += image[w+x][h+y][c+1] * kernelsAsFloats[m][x][y][c+1];
       	      sum += image[w+x][h+y][c+2] * kernelsAsFloats[m][x][y][c+2];
              sum += image[w+x][h+y][c+3] * kernelsAsFloats[m][x][y][c+3];
              sum += image[w+x][h+y][c+4] * kernelsAsFloats[m][x][y][c+4];
              sum += image[w+x][h+y][c+5] * kernelsAsFloats[m][x][y][c+5];
              sum += image[w+x][h+y][c+6] * kernelsAsFloats[m][x][y][c+6];
              sum += image[w+x][h+y][c+7] * kernelsAsFloats[m][x][y][c+7];
              sum += image[w+x][h+y][c+8] * kernelsAsFloats[m][x][y][c+8];
              sum += image[w+x][h+y][c+9] * kernelsAsFloats[m][x][y][c+9];
              sum += image[w+x][h+y][c+10] * kernelsAsFloats[m][x][y][c+10];
              sum += image[w+x][h+y][c+11] * kernelsAsFloats[m][x][y][c+11];
              sum += image[w+x][h+y][c+12] * kernelsAsFloats[m][x][y][c+12];
              sum += image[w+x][h+y][c+13] * kernelsAsFloats[m][x][y][c+13];
              sum += image[w+x][h+y][c+14] * kernelsAsFloats[m][x][y][c+14];
              sum += image[w+x][h+y][c+15] * kernelsAsFloats[m][x][y][c+15];
            }
          }
          output[m][w][h] = (float)sum;
        }
...
int main(int argc, char **argv)
{
  ...
  long long mul_time_improved;
  long long mul_time_original;
  int width, height, kernel_order, nchannels, nkernels;
  struct timeval start_time_improved;
  struct timeval stop_time_improved;
  struct timeval start_time_original;
  struct timeval stop_time_original;
  ...
  gettimeofday(&start_time_original, NULL);
  /* use a simple multichannel convolution routine to produce control result */
  multichannel_conv(image, kernels, control_output, width,
                    height, nchannels, nkernels, kernel_order);

  gettimeofday(&stop_time_original, NULL);
  mul_time_original = (stop_time_original.tv_sec - start_time_original.tv_sec) * 1000000L + (stop_time_original.tv_usec - start_time_original.tv_usec);

  /* record starting time of team's code*/
  gettimeofday(&start_time_improved, NULL);
  ...
  gettimeofday(&stop_time_improved, NULL);
  mul_time_improved = (stop_time_improved.tv_sec - start_time_improved.tv_sec) * 1000000L +
             (stop_time_improved.tv_usec - start_time_improved.tv_usec);
  printf("\nThe original conv time: %lld microseconds\n", mul_time_original);
  printf("The student conv time (improved): %lld microseconds\n", mul_time_improved);
  printf("The speedup: x%lld\n\n", mul_time_original / mul_time_improved);
